...
import { Camera, Volume2, VolumeX, Play, Pause, ListOrdered, RotateCcw, Mic, MicOff } from 'lucide-react';
...
export const BroadcastControls: React.FC<BroadcastControlsProps> = ({
  ...
  const cameraOptions: { mode: CameraMode; label: string; icon: string }[] = [
    ...
    { mode: 'wide', label: 'Aerial Drone Overview', icon: '🚁' },
  ...
  return (
    <nav
      ...
      className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-slate-900/90 backdrop-blur-md border border-slate-800/90 rounded-3xl p-1.5 px-3 shadow-2xl z-30 pointer-events-auto ring-1 ring-white/5"
    >
      ...
      <div className="relative">
        ...
        >
          ...
          <span className="hidden sm:inline text-[11px] text-slate-400">Cam:</span>
          <span className="capitalize text-slate-200 font-extrabold text-[11px]">
            {currentCameraMode === 'broadcast' ? 'Auto' : currentCameraMode}
          </span>
        </button>

        {cameraMenuOpen && (
          <div className="absolute bottom-full mb-2 left-0 w-56 bg-slate-900/95 backdrop-blur-md border border-slate-800/90 rounded-3xl p-2 shadow-2xl space-y-1 ring-1 ring-white/5">
            <div className="text-[9px] uppercase font-black tracking-widest text-slate-400 px-2 py-1">
      ...
      <div className="flex items-center bg-slate-950/80 rounded-2xl p-0.5 border border-slate-800/80">
        {[1, 1.5, 2, 4].map(spd => (
          <button
            key={spd}
      ...
      <button
        ...
        className="p-2 rounded-2xl bg-slate-950/80 hover:bg-slate-800 text-slate-300 border border-slate-800/80 transition-all shadow-sm"
      ...
      <button
        ...
        className={`p-2 rounded-2xl border transition-all shadow-sm ${
          isMuted
            ...
            : 'bg-slate-950/80 hover:bg-slate-800 border-slate-800/80 text-slate-300'
        }`}
      ...
      {/* Speech Commentary Voice Toggle */}
      <button
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl bg-slate-950/80 hover:bg-slate-800 text-slate-200 border border-slate-800/80 text-xs font-bold transition-all shadow-sm"
      >
        <ListOrdered className="w-4 h-4 text-amber-400" />
        <span className="hidden sm:inline text-[11px]">Tournament</span>
      </button>
      ...
      <button
        ...
        className="p-2 rounded-2xl bg-slate-950/80 hover:bg-rose-900/40 text-slate-400 hover:text-rose-300 border border-slate-800/80 transition-all shadow-sm"
